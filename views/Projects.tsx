...
interface Project {
  ...
  templateType?: 'react' | 'node' | 'static';
}
...
const Projects: React.FC<ProjectsProps> = ({ onNavigate }) => {
  ...
  const handleOpenProject = (project: Project) => {
    ...
    if (project.templateType === 'node') {
      template = NODE_TEMPLATE;
    } else if (project.templateType === 'static') {
               ...
               {projects.map(p => (
                 <div key={p.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-zinc-700 transition-all group shadow-xl">
                    ...
                    <div className="p-5">
                       ...
                       >
                          <span className="material-symbols-rounded text-lg">code</span> Open in Workspace
                       </button>
                    </div>
                 </div>
               ))}
