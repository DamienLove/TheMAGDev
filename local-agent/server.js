...
const PORT = 4477;
const ALLOWED_ORIGINS = ['http://localhost:3000', 'http://localhost:5173', 'https://themag.dev', 'https://stackblitz.io'];

const wss = new WebSocketServer({
    port: PORT,
    verifyClient: (info, cb) => {
        const origin = info.origin;
        if (!origin || ALLOWED_ORIGINS.some(allowed => origin === allowed || origin.startsWith('http://localhost:'))) {
            cb(true);
        } else {
            console.warn(`[Security] Blocked unauthorized connection from origin: ${origin}`);
