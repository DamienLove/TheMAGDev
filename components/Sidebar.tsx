    ...
    <aside className="w-16 flex flex-col items-center bg-slate-900 border-r border-slate-800 py-4 h-full shrink-0 z-50">
      ...
      <nav className="flex-1 flex flex-col gap-4 w-full px-2 overflow-y-auto no-scrollbar">
        {navItems.map((item) => (
          ...
          >
            <span aria-hidden="true" className="material-symbols-rounded">{item.icon}</span>
            {/* Tooltip */}
      ...
      <div className="flex flex-col gap-4 w-full px-2 mt-4 pt-4 border-t border-slate-800">
        ...
        >
          <span aria-hidden="true" className="material-symbols-rounded">contact_support</span>
          <div className="absolute left-14 bg-slate-800 text-slate-200 text-xs px-2 py-1 rounded border border-slate-700 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50 shadow-xl">
            Support
          </div>
        </button>
        ...
        <button
          ...
          className="w-10 h-10 mx-auto rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-xs font-bold border-2 border-slate-950 shadow-lg cursor-pointer hover:scale-110 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
        >
          {user?.avatar || 'JD'}
        </button>
      </div>
