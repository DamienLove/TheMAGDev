...
const FileExplorer: React.FC<FileExplorerProps> = ({ className, onPopOut, onOpenWindow, hideHeader, hideProjectName, projectName }) => {
  const {
    files,
    activeFile,
    openFile,
      ...
      {!hideProjectName && (
        <div className="px-3 py-2 border-b border-zinc-800/30">
          <div className="flex items-center gap-1.5 text-zinc-200">
            ...
            <span className="text-xs font-bold uppercase tracking-tight">{projectName || 'themag-framework'}</span>
          </div>
        </div>
      )}
