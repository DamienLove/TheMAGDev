...
export const WorkspaceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getFileByPath = useCallback((path: string): FileNode | undefined => {
    ...
    // Replaced O(N) eager map generation with O(log N) lazy tree traversal.
    // Rebuilding a Map on every file keystroke (files state change) blocks the main thread.
    // Path pruning ensures we only traverse branches containing the target file.
