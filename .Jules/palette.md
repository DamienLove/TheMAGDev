...
## 2025-05-19 - [Form Accessibility in Auth Flows]
**Learning:** Screen reader users can be blocked from authentication if form inputs lack programmatic association with their visible labels.
**Action:** Always use `useId()` to generate unique IDs and link `<label htmlFor={id}>` with `<input id={id}>` to ensure accessible names are computed correctly.
