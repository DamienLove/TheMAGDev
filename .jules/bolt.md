...
## 2026-05-20 - [Parallelize computeGitBlobSha for Git Sync]
**Learning:** Sequential await inside a loop for `computeGitBlobSha` creates a bottleneck in file diff computation during Git Sync operations when dealing with many files.
**Action:** Always use `Promise.all` to parallelize independent cryptographic operations or hash computations instead of executing them sequentially in a `for...of` loop.
