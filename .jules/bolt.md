...
## 2025-03-09 - [Early Returns for Referentially Equal React State (Sets)]
**Learning:** When using `new Set(prev)` to create a new Set for React state updates (like `setUnsavedFiles` or `setExpandedFolders`), if the specific item to be added/deleted doesn't change the underlying contents of the Set, it still breaks referential equality and forces an unnecessary re-render of components subscribed to that state. In large applications, this can be extremely inefficient.
**Action:** When working with Set or Map states in React, always check if an update is actually necessary (e.g., `prev.has(x)`) and if it's not, return the original reference (`prev`) to preserve referential equality.
