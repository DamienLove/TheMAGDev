...
## 2026-03-22 - [Set/Map Referential Equality in High-Frequency React State]
**Learning:** When using mutable data structures like `Set` or `Map` in React state (e.g., `unsavedFiles`), updating state with `new Set(prev)` without an early return check (like `prev.has(path)`) breaks referential equality on every high-frequency event like keystrokes. This triggers unnecessary downstream component invalidations and massive React rendering overhead, even when the underlying data hasn't logically changed.
**Action:** Always implement early returns for Set and Map state updates (e.g., `setUnsavedFiles(prev => prev.has(path) ? prev : new Set(prev).add(path))`). This preserves referential equality and correctly bails out of rendering when the collection doesn't change.
