...
## 2024-03-27 - [Early Returns in Set/Map State Updates]
**Learning:** State updates for Set and Map structures (like `unsavedFiles` in WorkspaceContext) must implement early returns (e.g., `prev.has(x) ? prev : new Set(prev).add(x)`) to preserve referential equality and prevent unnecessary downstream React component invalidations during high-frequency events. Without this, creating a new `Set` instance on every update triggers re-renders even if the contents haven't changed.
**Action:** Always check if a Map/Set needs modification before creating a new instance in a state setter callback.
