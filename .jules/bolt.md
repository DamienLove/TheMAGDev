...
## 2024-05-23 - [Early Returns for State Sets/Maps]
**Learning:** Unconditional new Set/Map instantiations for React state updates during high-frequency events (like typing) break referential equality and cause unnecessary downstream re-renders.
**Action:** Always implement early returns (e.g., `prev.has(x) ? prev : new Set(prev).add(x)`) for Set/Map state updates to preserve referential equality when the collection state has not actually changed.
