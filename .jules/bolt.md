...
\n## 2025-05-06 - [O(N) vs O(log N) Lazy Traversal in Nested State]\n**Learning:** In React components managing deeply nested file tree states, building a comprehensive O(N) lookup Map on every keystroke ( state change) inside a  introduces severe blocking on the main thread.\n**Action:** Replace eager indexing approaches with lazy O(log N) targeted tree traversal using path prefix pruning (e.g. ) to only scan necessary branches on-demand.

## 2024-05-06 - [O(N) vs O(log N) Lazy Traversal in Nested State]
**Learning:** In React components managing deeply nested file tree states, building a comprehensive O(N) lookup Map on every keystroke (`files` state change) inside a `useMemo` introduces severe blocking on the main thread.
**Action:** Replace eager indexing approaches with lazy O(log N) targeted tree traversal using path prefix pruning (e.g. `path.startsWith(node.path + '/')`) to only scan necessary branches on-demand.
