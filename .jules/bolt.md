...
## 2026-04-26 - Parallel Git Blob Hashing
**Learning:** During workspace file git status checks in `views/CodeEditor.tsx`, hashing local files using `computeGitBlobSha` sequentially via an `await` within a `for...of` loop is unnecessarily slow because the `crypto.subtle.digest` operations are entirely independent and I/O (microtask) bound.
**Action:** Always map these independent async validation or hashing operations into an array of Promises and execute them concurrently via `Promise.all` to significantly reduce total elapsed time.
