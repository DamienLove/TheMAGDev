...
## 2026-03-12 - [Set and Map State Reference Equality]
**Learning:** Frequent updates to Set or Map structures in React state (like `unsavedFiles` tracking on every keystroke) can unintentionally trigger widespread component re-renders if a new Set reference is returned unnecessarily (e.g., `new Set(prev).add(x)` when `x` is already in the Set).
**Action:** Always implement early returns for Set and Map state updates (`prev.has(x) ? prev : new Set(prev).add(x)`) to preserve referential equality and skip unnecessary React component validaitons during high-frequency events.
