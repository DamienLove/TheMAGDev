...
## 2024-05-22 - [Set/Map State Referential Equality in React]
**Learning:** React state updates using Sets (and Maps) like `setUnsavedFiles(prev => new Set(prev).add(path))` trigger re-renders even if the item already exists in the collection, because a new referential object (`new Set`) is created regardless of whether the collection fundamentally changed.
**Action:** When updating Set or Map states in React, always implement an early return check (e.g., `prev.has(path) ? prev : new Set(prev).add(path)` or `if (!prev.has(path)) return prev;`) to preserve referential equality when no actual mutation is needed, avoiding unnecessary re-renders downstream.
