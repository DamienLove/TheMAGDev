...
## 2026-03-25 - [Preserve referential equality for complex React state objects like Sets/Maps]
**Learning:** In a heavily memoized application like an IDE, returning a new instance of a `Set` or `Map` from a `setState` callback even when the underlying data hasn't changed (e.g., `new Set(prev).add(x)` when `x` is already in the set) triggers sweeping cascading re-renders across the entire component tree. This is especially problematic in high-frequency event handlers like file content updates (`updateFileContent`).
**Action:** When updating Set/Map structures in React state (like `unsavedFiles` in WorkspaceContext), always implement early returns (e.g., `prev.has(x) ? prev : new Set(prev).add(x)`) to preserve referential equality and prevent unnecessary downstream React component invalidations.
