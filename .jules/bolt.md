...
## 2026-05-07 - [Path Pruning for Recursive React Context Updates]
**Learning:** In highly nested data structures like `FileNode[]` stored in React state, rebuilding a global O(N) lookup Map on every state change using `useMemo` triggers full tree traversals, blocking the main thread during frequent updates like typing in the editor.
**Action:** Replaced eager mapping with a lazy, on-demand O(log N) tree traversal in `getFileByPath` using path prefix pruning (e.g., `if (targetPath.startsWith(node.path + '/'))`). This bypasses unnecessary directory branches completely.
