...
## 2026-03-14 - [Replacing O(N) Map Memoization with O(depth) Traversal]
**Learning:** In `WorkspaceContext`, maintaining a flattened `fileMap` via `useMemo([files])` triggered a full O(N) tree traversal on *every* file content update, causing severe input lag during typing. Memoization is an anti-pattern when the source structure updates constantly and the lookup algorithm is trivial.
**Action:** Replaced the full-tree map with an O(depth) path-splitting traversal in `getFileByPath`. This eliminates the expensive recalculation entirely while maintaining logarithmic lookup speed, significantly improving IDE responsiveness.
