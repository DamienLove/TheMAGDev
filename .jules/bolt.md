...
## 2024-05-22 - [Parallelize computeGitBlobSha to improve performance]
**Learning:** Calling `crypto.subtle.digest` sequentially in a loop blocks and delays processing. Parallelizing these asynchronous operations using `Promise.all` improves git change detection performance significantly.
**Action:** When computing cryptographic hashes or performing other async operations iteratively on independent data, use array mapping and `Promise.all` rather than awaiting sequentially in a `for...of` loop.
