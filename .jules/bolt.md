...
## 2026-03-28 - [Set/Map Referential Equality in React State]
**Learning:** State updates for Set and Map structures (like `unsavedFiles` in WorkspaceContext) should implement early returns (e.g., `prev.has(x) ? prev : new Set(prev).add(x)`) to preserve referential equality and prevent unnecessary downstream React component invalidations during high-frequency events.
**Action:** When working with Set or Map in React state, always check if the structure already contains (or doesn't contain) the target item before creating a new Set/Map instance.
