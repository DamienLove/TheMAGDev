...
## 2026-05-11 - [O(N) useMemo anti-pattern on recursive state]
**Learning:** Using `useMemo` to build a lookup Map for deeply nested tree state arrays (like `FileNode[]`) results in a catastrophic O(N) traversal on every single state change (e.g., every keystroke in a file). Eager indexing is an anti-pattern when the data structure updates frequently.
**Action:** Replace eager O(N) indexing with lazy O(log N) tree traversal methods that utilize branch pruning (e.g., `targetPath.startsWith(node.path + '/')`). Only build lookups for read-heavy, infrequently changing static data.
