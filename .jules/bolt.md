...
## 2026-04-07 - [Early Returns in Set/Map State Updates]
**Learning:** In React components, updating `Set` or `Map` state structures without checking for existing values (e.g., `new Set(prev).add(x)` even if `x` is already present) creates a new reference. This breaks referential equality and forces unnecessary downstream re-renders, especially impactful in large context providers like `WorkspaceContext`.
**Action:** Always implement early returns for Set and Map state updates (e.g., `prev.has(path) ? prev : new Set(prev).add(path)` and `if (!prev.has(path)) return prev`) to preserve referential equality when the state functionally does not change.
