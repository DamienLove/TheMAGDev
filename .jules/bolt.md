...
## 2025-03-09 - [FileMap O(N) Traversal Optimization]
**Learning:** Rebuilding a hash map of an entire tree structure (`fileMap`) on every keystroke defeats the purpose of path pruning in state updates. Even if state updates are optimized to O(log N), `useMemo` rebuilding the map takes O(N) time and blocks the main thread in large workspaces.
**Action:** Replace full-tree map memoization with an O(depth) path-based tree traversal function (`getFileByPath`). This removes the O(N) cost on every file edit and provides practically O(1) performance since the depth of files is small.
