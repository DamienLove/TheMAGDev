...
## 2026-03-18 - [Set State Referential Equality]
**Learning:** In React, updating a state variable that holds a `Set` by constantly returning a `new Set(prev)` forces re-renders of the component and all context consumers, even if no items were actually added or removed.
**Action:** When updating Set or Map structures in state, implement early returns (e.g., `if (!prev.has(x)) return prev` or `prev.has(x) ? prev : new Set(prev).add(x)`) to preserve referential equality when the collection doesn't logically change, thereby preventing wasteful React rendering cycles.
