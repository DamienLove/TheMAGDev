...
## 2024-05-24 - [Local Agent CSWSH Fix]
**Vulnerability:** The local WebSocket server (`local-agent/server.js`) lacked Origin verification (`verifyClient`).
**Learning:** This could allow a malicious website to connect via the user's browser to the local WebSocket server and execute arbitrary commands on their machine (Remote Code Execution via Cross-Site WebSocket Hijacking).
**Prevention:** Always implement a `verifyClient` callback in `WebSocketServer` configurations that strictly whitelists allowed origins (e.g., `http://localhost:*`, `https://themag.dev`, or `undefined` for non-browser local scripts).
