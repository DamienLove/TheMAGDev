...
## 2025-03-09 - Cross-Site WebSocket Hijacking in root local-agent
**Vulnerability:** The root level local-agent server (`local-agent/server.js`) did not validate the `Origin` header.
**Learning:** Similar to the script inside `scripts/`, `ws` does not validate origins by default. Both versions of local agent are vulnerable to CSWSH/RCE without `verifyClient`.
**Prevention:** Ensure any local server utilizing `ws` enforces `Origin` verification for programmatic or browser interactions.
