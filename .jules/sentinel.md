...
## 2025-03-09 - Cross-Site WebSocket Hijacking in Local Agent Servers
**Vulnerability:** The local agent server in `local-agent/server.js` lacked any validation of the `Origin` header. Since browsers automatically include the `Origin` header but don't enforce the Same-Origin Policy on WebSocket connections natively, this omission allowed any website to connect to the local WebSocket server (`ws://localhost:4477`) and potentially execute arbitrary shell commands on a developer's machine via Cross-Site WebSocket Hijacking (CSWSH).
**Learning:** Even local development servers handling WebSockets need explicitly whitelist-based origin validation. Developers often skip this for local tools, creating massive security holes if a developer inadvertently visits a malicious site while their local environment is running.
**Prevention:** Consistently use the `verifyClient` callback during the initialization of `WebSocketServer` instances (like from the `ws` library) to validate that incoming `Origin` headers strictly match anticipated sources (e.g., specific domains or `localhost` ports).
