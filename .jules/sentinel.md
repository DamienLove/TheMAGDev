...
## 2025-03-09 - Cross-Site WebSocket Hijacking in Raw Shell Agent
**Vulnerability:** The raw shell local agent server (`local-agent/server.js`) did not validate the `Origin` header, exposing an RCE vector.
**Learning:** Raw shell agent connections that accept WebSockets on a local port must be secured against CSWSH. Programmatic connections like Node scripts send an undefined origin, which must be explicitly allowed alongside strict dynamic port prefix matching for local development.
**Prevention:** Always implement `verifyClient` for `ws` WebSocketServer, correctly handling undefined origins for programmatic clients and strictly validating browser origins.
