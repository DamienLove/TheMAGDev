...
## 2024-05-14 - Prevent Cross-Site WebSocket Hijacking in local agents
**Vulnerability:** The local WebSocket agent server (`local-agent/server.js`) accepted connections from any origin, allowing any website to connect and execute arbitrary shell commands on the user's machine (CSWSH leading to RCE).
**Learning:** The default behavior of the `ws` library's `WebSocketServer` does not check the `Origin` header. It assumes all cross-origin connections are allowed unless explicitly restricted.
**Prevention:** Always implement the `verifyClient` option when initializing `WebSocketServer` to validate the `Origin` header against a whitelist of expected origins.
