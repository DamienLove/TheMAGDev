...
## 2025-03-09 - Incomplete Window.open Security Features
**Vulnerability:** External links opened using `window.open` lacked the `noreferrer` feature, only using `noopener`.
**Learning:** While `noopener` prevents the newly opened window from having a reference back to the original window (Tabnabbing), it does not prevent the Referer header from being sent. This can leak sensitive URLs or user information to third-party sites.
**Prevention:** Always use `noopener,noreferrer` for external links opened via `window.open`. Note that `noreferrer` should not be used for internal, same-origin popouts if the application relies on retaining a window reference for functionality.
