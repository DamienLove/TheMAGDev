...
**Vulnerability:** The local agent server (`local-agent/server.js`) did not validate the `Origin` header.
**Learning:** Without `verifyClient` in `WebSocketServer`, any website a user visits can connect to the local agent running on port 4477 and execute arbitrary shell commands.
**Prevention:** Always implement a `verifyClient` callback in `WebSocketServer` configurations to strictly whitelist allowed origins, allowing `undefined` for non-browser clients.
