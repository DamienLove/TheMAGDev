...
## 2026-01-30 - Prevent CSWSH Remote Code Execution via WebSockets
**Vulnerability:** The local agent server script (`local-agent/server.js`) started a WebSocket server that spawned raw interactive shells (`bash`, `powershell.exe`) and bound them directly without verifying the `Origin` header. This allowed malicious external websites to connect to the agent port locally (Cross-Site WebSocket Hijacking - CSWSH) and achieve Remote Code Execution (RCE).
**Learning:** By default, the `ws` package does not check or validate origins. When running tools that provide powerful native access (like shell spawning), exposing them blindly on localhost creates severe critical vulnerabilities because any browser can make `ws://` connections to `localhost`.
**Prevention:** Always implement the `verifyClient` callback on raw `WebSocketServer` instances, especially those performing high-privilege operations. Ensure that the Origin header is explicitly checked to match your expected domain or explicit local bounds (e.g. `http://localhost:*` combined with ensuring non-web programmatic clients pass `origin: undefined`).
