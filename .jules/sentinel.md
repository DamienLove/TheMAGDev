...
## 2025-03-09 - Cross-Site WebSocket Hijacking in Secondary Local Agent
**Vulnerability:** The local agent server (`local-agent/server.js`) did not validate the `Origin` header, allowing any website the user visits to connect to the agent and execute arbitrary shell commands on their machine.
**Learning:** This CSWSH vulnerability was fixed previously in another script, but the pattern was repeated in the secondary script. Developers need to be reminded that WebSocket connections automatically include the origin header but need explicit validation.
**Prevention:** Always implement a `verifyClient` callback in `WebSocketServer` configurations that strictly whitelists allowed origins (e.g., `http://localhost:3000`, `https://themag.dev`).
