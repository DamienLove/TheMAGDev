...
## 2025-05-01 - Local Agent Dynamic Port CSWSH
**Vulnerability:** The origin check in `scripts/local-agent.js` was hardcoded to specific localhost ports, which fails to support dynamic ports, and `local-agent/server.js` lacked origin validation entirely.
**Learning:** Both local agent scripts must implement dynamic `localhost` origin validation (e.g. `origin.startsWith('http://localhost:')`) to secure against CSWSH without breaking legitimate local dev environments that might bind to different ports.
**Prevention:** Always implement a `verifyClient` callback in all local WebSocket servers that securely pattern matches local origins in addition to whitelisted production domains.
