...
## 2024-05-23 - Cross-Site WebSocket Hijacking in local-agent package
**Vulnerability:** Similar to the root local agent, the separate `local-agent/server.js` implementation also lacked `Origin` header validation.
**Learning:** Vulnerabilities are often duplicated across different implementations or packages within a monorepo. When fixing an issue in one place, always search for other instances of the same pattern.
**Prevention:** Establish a single shared configuration or utility for critical security configurations (like `WebSocketServer` initialization) to ensure all instances are consistently protected.
