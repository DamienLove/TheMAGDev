...
## 2026-04-19 - Fix CSWSH vulnerability in WebSocket servers
**Vulnerability:** Cross-Site WebSocket Hijacking (CSWSH) and potential Remote Code Execution (RCE) via local WebSocket servers.
**Learning:** The 'ws' library does not validate the 'Origin' header by default. When raw shells are spawned and piped directly to WebSockets, this can allow malicious websites to connect to the local server and execute arbitrary commands.
**Prevention:** Always implement a `verifyClient` callback in `WebSocketServer` configurations that strictly whitelists allowed origins (e.g., `http://localhost:3000`, `https://themag.dev`) or validates local/authorized connections.
