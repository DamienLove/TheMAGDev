...
## 2025-02-27 - [CSWSH in Raw WebSockets]
**Vulnerability:** The local agent in `local-agent/server.js` spawned a raw shell (bash/powershell) and piped stdin/stdout directly to a `WebSocketServer` created via the `ws` package without validating the connection `origin`. Because it listened on a local port (4477), any malicious website a developer visited could connect directly to `ws://localhost:4477` and execute arbitrary system commands (RCE) on the developer's machine via Cross-Site WebSocket Hijacking (CSWSH).
**Learning:** The `ws` library does not validate the `Origin` header by default, unlike standard CORS behavior for HTTP endpoints. When building local tools that spawn raw shells over WebSockets, the absence of origin validation is equivalent to an unauthenticated RCE backdoor for local development.
**Prevention:** Always implement the `verifyClient` callback in the `WebSocketServer` configuration to explicitly enforce allowed origins. Ensure you allow `undefined` origins for non-browser programmatic clients (like Node scripts), use pattern matching for dynamic ports (e.g., `origin.startsWith('http://localhost:')`), and explicitly define any external domains required for integration.
