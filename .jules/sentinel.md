...
## 2025-03-09 - Cross-Site WebSocket Hijacking in Multiple Local Agents
**Vulnerability:** The `local-agent/server.js` script did not validate the `Origin` header, similar to the previously fixed `scripts/local-agent.js`. This allowed any website the user visits to connect to the agent and execute arbitrary shell commands.
**Learning:** When a vulnerability is found in one place (e.g., a local agent script), it's crucial to search the entire codebase for duplicate or similar implementations. The same vulnerability often exists in multiple files due to copy-pasting or parallel development.
**Prevention:** Always implement a `verifyClient` callback with strict origin whitelisting in *all* `WebSocketServer` configurations. Ensure codebase-wide searches are performed when a critical vulnerability pattern is discovered.
