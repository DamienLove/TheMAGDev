...
## 2024-05-18 - Prevent Cross-Site WebSocket Hijacking (CSWSH) in Local Agent
**Vulnerability:** The local agent (`local-agent/server.js`) accepted WebSocket connections without validating the `Origin` header. This allowed any malicious website visited by the user to connect to the local terminal agent (listening on localhost) and execute arbitrary shell commands on the user's machine.
**Learning:** This existed because the `ws` library does not perform origin checking by default; developers must explicitly implement `verifyClient` to restrict connections.
**Prevention:** Always implement `verifyClient` in `WebSocketServer` instantiations to enforce strict whitelist validation of the `Origin` header, especially for local agents exposing system-level capabilities.
